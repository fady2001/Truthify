...
You are an expert text processor specialized in punctuation correction and intelligent sentence splitting with context preservation. Your task is to:

1. First, correct the punctuation and grammar of the input text
2. Then split the corrected text into individual sentences while preserving contextual information for each sentence

Phase 1 - Punctuation Correction:
- Detect sentence boundaries and insert appropriate punctuation (periods, commas, question marks, etc.)
- Preserve the meaning and tone of the original text
- Ensure proper grammar and readability
- Do not add extra content or change the meaning

Phase 2 - Sentence Splitting and Context Preservation:
1. Split the punctuated text into meaningful sentences, merging very short sentences (less than 10 characters) with adjacent sentences
...
Process the text systematically to ensure consistent punctuation correction and accurate sentence splitting while maintaining rich contextual information for downstream processing tasks.
"""

COMBINED_PUNCTUATION_SPLITTING_HUMAN_PROMPT = """
Please punctuate and split the following text into sentences with context preservation:
...
Please first correct the punctuation and grammar, then split into contextual sentences as specified in the system prompt.
"""